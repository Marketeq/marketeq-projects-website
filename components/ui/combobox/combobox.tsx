...
interface ComboboxProps<TValue> extends Omit<
...
export const comboboxButtonVariants = cva(
  "inline-flex justify-center items-center absolute inset-y-0 my-auto text-gray-400 peer-hover:text-gray-500 peer-focus:text-primary-500 peer-disabled:cursor-not-allowed peer-disabled:opacity-50 focus-visible:outline-hidden",
  {
    variants: {
      align: {
        left: "",
...
interface ComboboxButtonProps extends WithoutChildren<
...
interface ComboboxOptionsProps extends WithoutChildren<
...
>(({ className, ...props }, ref) => (
  <ComboboxPrimitive.Options
    className={cn(
      "absolute mt-1 w-full z-50 space-y-1 rounded-[5px] bg-white px-1 shadow-2xl",
      className
    )}
    {...props}
    ref={ref}
...
interface ComboboxOptionProps<TValue = any> extends Omit<
