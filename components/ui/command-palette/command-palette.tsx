...
import { callAll, cn } from "@/utils/functions"
...
>(({ className, variant, ...props }, ref) => (
  <div className="mt-8 px-6">
    <div className="relative" cmdk-input-wrapper="">
      <SearchMd className="absolute left-3.5 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-500" />
...
>(({ className, ...props }, ref) => (
  <CommandPalettePrimitive.Group
    ref={ref}
    className={cn(
      "overflow-hidden pb-6 **:[[cmdk-group-heading]]:mb-3 **:[[cmdk-group-heading]]:text-xs **:[[cmdk-group-heading]]:font-medium **:[[cmdk-group-heading]]:uppercase **:[[cmdk-group-heading]]:tracking-[0.02em] **:[[cmdk-group-heading]]:text-gray-500 **:[[cmdk-group-items]]:grid **:[[cmdk-group-items]]:gap-3 md:**:[[cmdk-group-items]]:grid-cols-2",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <CommandPalettePrimitive.Item
      ...
      className={cn(
        "flex h-[68px] cursor-pointer select-none items-center gap-x-3 rounded-lg border border-gray-200 p-[13px] shadow-[0px_1px_4px_0px_rgba(0,0,0,0.03)] outline-hidden transition duration-300 hover:border-2 hover:border-gray-300 hover:p-3",
        className
      )}
      {...props}
    />
