...
import { callAll, cn, isArray, noop } from "@/utils/functions"
...
interface DropdownPopoverProps extends React.ComponentPropsWithoutRef<
...
>(({ className, ...props }, ref) => (
  <PopoverPrimitives.Trigger
    className={cn(
      "flex h-11 w-full items-center rounded-[5px] border border-gray-300 bg-white p-2.5 pl-3 text-base text-gray-800 data-[state=open]:border-primary-400 data-[state=open]:ring-4 data-[state=open]:ring-primary-50 focus-visible:outline-hidden",
      className
    )}
    {...props}
    ref={ref}
...
>(
  (
    ...
    <PopoverPrimitives.PopoverContent
      className={cn(
        "relative z-50 rounded-lg bg-white shadow-[0px_4px_25px_0px_rgba(0,0,0,0.1)] outline-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
        className
      )}
      side={side}
      align={align}
